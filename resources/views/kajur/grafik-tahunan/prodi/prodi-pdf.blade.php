...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        <a href="<?= url('/kajur/hasil-evaluasi/dosen/prodi/'. $prodi_id .'/grafik') ?>" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm"><i class="fas fa-chart-bar fa-sm text-white-50"></i> Tampilan Grafik</a>
        <a id="pdf-button" target="_blank" target="_blank" href="<?= url('/kajur/hasil-evaluasi/dosen/prodi/'. $prodi_id .'/pdf') ?>" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm"><i class="fas fa-file-pdf fa-sm text-white-50"></i> Export to pdf</a>
    </div>
    ...
    const tampilData = data => {
        ...
        data.dosen.forEach(dt => {
            const el = `<tr>
                    ...
                    <td> <a href="<?= url('/kajur/hasil-evaluasi/dosen/dosen/${dt.id}') ?>"><i class="fas fa-search-plus" ></i></a> </td>
                </tr>`;
            $('#dosen').append(el);
        ...
        $('#pdf-button').attr('href',`<?= url('/kajur/hasil-evaluasi/dosen/prodi/' . $prodi_id . '/pdf?tahun_id=${data.tahun_id}') ?>`);
