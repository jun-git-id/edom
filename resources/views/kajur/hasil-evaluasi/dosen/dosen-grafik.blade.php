...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        <a href="<?= url('/kajur/hasil-evaluasi/dosen/dosen-pert/' . $dosen_id) ?>" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm"><i class="fas fa-sticky-note fa-sm text-white-50"></i> Per Pertanyaan</a>
        <a href="<?= url('/kajur/hasil-evaluasi/dosen/dosen/' . $dosen_id) ?>" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm"><i class="fas fa-table fa-sm text-white-50"></i> Tampilan Tabel</a>
...
<script>
    const url = "<?= url('/api/admin/hasil-evaluasi/dosen/dosen/' . $dosen_id) ?>";
    ...
    $('#tampilkan-btn').click(e => {
        ...
        const url2 = `<?= url('/api/admin/hasil-evaluasi/dosen/dosen/' . $dosen_id . '?tahun_id=${thn_id}') ?>`;
        $.get(url2, data => {
    ...
    const tampilData = data => {
        ...
        data.ajaran.forEach(dt => {
            const el = `<tr>
                            ...
                            <td> <a href="<?= url('/kajur/hasil-evaluasi/dosen/ajaran/${dt.id}') ?>"><i class="fas fa-search-plus"></i></a> </td>
                        </tr>`;
            $('#ajaran').append(el);

            i++;
            arr.push(parseFloat(dt.nilai));
            label_gr.push(dt.kelas);
        ...
        $('#pdf-button').attr('href', `<?= url('/kajur/hasil-evaluasi/dosen/dosen/' . $dosen_id . '/pdf?tahun_id=${data.tahun_id}&grafik=yes') ?>`);
